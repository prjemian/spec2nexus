...
class SpecPlotGallery(unittest.TestCase):
    ...
    def test_command_line_spec_data_file_33bm_spec(self):
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    self.tempdir, specplot_gallery.MTIME_CACHE_FILE
                )
            )
        )
        ...
        self.assertTrue(
            os.path.exists(os.path.join(plotDir, "33bm_spec.dat"))
        )
    ...
    def test_command_line_spec_data_file_user6idd(self):
        ...
        self.assertTrue(os.path.exists(plotDir))
        self.assertTrue(
            os.path.exists(os.path.join(plotDir, "user6idd.dat"))
        )
        self.assertTrue(
            os.path.exists(os.path.join(plotDir, "index.html"))
        )
    ...
    def test_command_line_spec_data_file_03_06_JanTest(self):
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    plotDir, "s00001" + specplot_gallery.PLOT_TYPE
                )
            )
        )
        # TODO: #69: look for handling of scan 1
        self.assertFalse(
            os.path.exists(
                os.path.join(plotDir, "s1" + specplot_gallery.PLOT_TYPE)
            )
        )
    ...
    def test_command_line_spec_data_file_list(self):
        ...
        for (
            item
        ) in "user6idd.dat APS_spec_data.dat 02_03_setup.dat".split():
    ...
    def test_command_line_specified_directory_not_found_issue_98(self):
        ...
        sys.argv.append(self.abs_data_fname("APS_spec_data.dat"))
        self.assertRaises(
            specplot_gallery.DirectoryNotFoundError, specplot_gallery.main
        )
    ...
    def test_command_line_specified_directory_fails_isdir_issue_98(self):
        ...
        sys.argv.append(self.abs_data_fname("APS_spec_data.dat"))
        self.assertRaises(
            specplot_gallery.PathIsNotDirectoryError, specplot_gallery.main
        )
...
class TestFileRefresh(unittest.TestCase):
    ...
    def test_refresh(self):
        specplot_gallery.PlotSpecFileScans([self.data_file], self.gallery)
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    self.gallery, specplot_gallery.MTIME_CACHE_FILE
                )
            )
        )
        ...
        for k in children:
            # should pass all
            self.assertGreater(
                os.path.getmtime(os.path.join(plotdir, k)), t0, k
            )
        ...
        specplot_gallery.PlotSpecFileScans(
            [self.data_file], self.gallery, reverse_chronological=True
        )
        self.assertEqual(len(children), 3)
        self.addMoreScans()
        time.sleep(0.1)
