...
class TestPlugin(unittest.TestCase):
    ...
    def test_process(self):
        ...
        self.assertTrue(
            isinstance(uxml, spec2nexus.plugins.uxml.UXML_metadata)
        )
        ...
        self.assertEqual(
            self.scan.postprocessors["UXML_metadata"], uxml.postprocess
        )
    ...
    def test_writer(self):
        ...
        self.assertTrue(
            hasattr(self.scan, "UXML_root"), "need to test writer()"
        )
        root = self.scan.UXML_root
...
class TestData_error(unittest.TestCase):
    def setUp(self):
        ...
        self.fname = os.path.join(
            _test_path, "tests", "data", "test_3_error.spec"
        )
    ...
    def testName(self):
        ...
        with self.assertRaises(
            spec2nexus.plugins.uxml.UXML_Error
        ) as context:
            scan.interpret()
...
class TestData_3(unittest.TestCase):
    def setUp(self):
        self.basepath, self.uxml_path = get_paths()
        self.fname = os.path.join(
            _test_path, "tests", "data", "test_3.spec"
        )
        basename = os.path.splitext(self.fname)[0]
        self.hname = basename + ".hdf5"
    ...
    def test_contents(self):
        ...
        with h5py.File(self.hname, "r") as h5_file:
            ...
            value = ds[()]  # ds.value deprecated in h5py
            self.assertEqual(value, numpy.array([0.5]))
...
class TestData_4(unittest.TestCase):
    def setUp(self):
        self.basepath, self.uxml_path = get_paths()
        self.fname = os.path.join(
            _test_path, "tests", "data", "test_4.spec"
        )
        basename = os.path.splitext(self.fname)[0]
        self.hname = basename + ".hdf5"
