...
class Test(unittest.TestCase):
    ...
    def test_strip_first_word(self):
        self.assertEqual(
            utils.strip_first_word("one two three"), "two three"
        )

    def test_isSpecFileThis(self):
        self.assertFalse(spec.is_spec_file("this_does_not_exist"))
        self.assertFalse(
            spec.is_spec_file(os.path.join(_path, "spec2nexus"))
        )
        ...
        self.assertFalse(
            spec.is_spec_file_with_header("file does not exist")
        )
        self.assertTrue(
    ...
    def test_extra_control_line_content__issue109(self):
        specFile = os.path.join(
            os.path.dirname(__file__), "data", "issue109_data.txt"
        )
        ...
        self.assertEqual(
            scan.data["Seco nds"][0], 1, "received expected count time"
        )
        self.assertNotEqual(
            scan.T,
        ...
        self.assertTrue(scan is not None)
        self.assertEqual(
            scan.M, "400000", "received expected monitor count"
        )
        ...
        self.assertEqual(
            len(scan.data[key]), 61, "embedded comment not part of data"
        )
