...
class Test(unittest.TestCase):
    ...
    def test_CdSe_scan_92(self):
        ...
        self.assertEqual(
            "# scan: 92", buf[1].strip(), "scan number in data file"
        )
        self.assertEqual(
            "# " + "\t".join(columns),
            buf[2].strip(),
            "column labels in data file",
        )

        # test first and last lines of data
        # remaining lines: three columns of numbers, tab delimited
        self.assertEqual(
            [-10.118571, 297.467, 66.0], list(map(float, buf[3].split()))
        )
        self.assertEqual(
            [-5.5910424, 297.483, 66.0], list(map(float, buf[-1].split()))
        )
    ...
    def test_CdSe_scan_95(self):
        ...
        self.assertEqual(
            "# scan: 95", buf[1].strip(), "scan number in data file"
        )
        self.assertEqual(
            "# " + "\t".join(columns),
            buf[2].strip(),
        ...
        # remaining lines: three columns of numbers, tab delimited
        self.assertEqual(
            [-12.063282, 297.529, 66.0], list(map(float, buf[3].split()))
        )
        self.assertEqual(
            [-2.0358687, 297.553, 66.0], list(map(float, buf[-1].split()))
        )
