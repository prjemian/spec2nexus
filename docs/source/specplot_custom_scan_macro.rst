...
of :class:`~spec2nexus.converters.PlotDataStructure` in attributes
show in the next table.  The data model is an adaptation of the 
NeXus *NXdata* base class. [#]_
...
creating a subclass of :class:`~spec2nexus.specplot.ImageMaker` and defining
one or more of its methods.  In the simplest case, certain settings may be
changed by calling :meth:`spec2nexus.specplot.ImageMaker.configure` with
...
of :class:`~spec2nexus.converters.PlotDataStructure` to gather the data to be plotted
or override the default :meth:`spec2nexus.specplot.ImageMaker.plottable` method.
An example of this is shown with the :class:`~spec2nexus.specplot.MeshPlotter` and 
associated :class:`~spec2nexus.converters.MeshStructure` classes.
 

Examples
********

A few exmaples of custom macro handling are provided, some simple, some complex.
...
We will show how to change the plot title as a means to illustrate 
how to customize the handling for a scan macro.

We write :class:`Custom_Ascan` which is a subclass of 
:class:`~spec2nexus.specplot.LinePlotter`.  The :mod:`get_plot_data` method is written 
...
the :mod:`configure` method (defined in the superclass).  Here's the code:
...
To do this, a custom version of :class:`~spec2nexus.specplot.LinePlotter`
is created as :class:`Custom_Ascan`.  The :mod:`get_plot_data` method is written 
...
Note that in the *uascan*, a name for the sample provided by the user 
is given in `self.scan.comments[0]`.  The plot title is changed to 
include this and the scan number.
The customized plot has a logarithmic *y* axis:
...
The most informative view of this data is when the raw data are
reduced to :math:`I(Q)` and viewed on a log-log plot,
...
vectors is scanned while the other two remain constant.

The normal handling of the *ascan* macro plots the last data column 
against the first.  This works for data collected with the *hscan*.  
For *kscan* or *lscan* macros, the *h* axis is still plotted by default 
since it is in the first column.
...
:class:`~spec2nexus.specplot.HKLScanPlotter` subclass,
provided with *specplot*, defines the :meth:`get_plot_data` method
